...
1.	Write up a <a href="./Example_Problems_that_ML_tools_should_be_better_at.pdf">short document</a> in LaTeX, defining the relevant notions and providing some context, in a manner that one might do for a human collaborator, and then feeding this document to an LLM as context.
...
The document describing the problem setup is <a href="./Example_Problems_that_ML_tools_should_be_better_at.pdf">here</a>. After the experiments above, I now believe that the converse to Lemma 2 document *is actually correct*, which *went against my expectations*. For me, this was the *first example* of an AI-assisted research process producing a result relevant to my actual research program which went *beyond literature search*. Links to my conversations with ChatGPT can be found <a href="https://chatgpt.com/share/68b244a4-f49c-8007-b406-c1acfe7cb356">here</a> and <a href="https://chatgpt.com/share/68b24509-cd78-8007-8792-221f762210f6">here</a>; the generated code can be found in these conversations, and the final code that changed my mind about Lemma 2 is <a href="./ai_generated_code.html">here</a>. In the figures below, I have highlighted instances of occurrences of the moments in the workflow described above.

The o4 model can do standard manipulations with differential forms:

<img src="./vibecoding_images/chatgpt_knows_differential_forms.png" class="inline-img" />

It can also write numerical code pretty well:

<img src="./vibecoding_images/IJK_convexity_numerical_test.png" class="inline-img" />

The model also quickly writes all kinds of algebraic manipulations which appear to prove the statements that I point out to it are provable in my document. Here is an apparent counterexample to Lemma 1, which feels like it requires some cleverness (*caveat: I did not check that the model's argument works!*)

<img src="./vibecoding_images/converse_to_lemma_1.png" class="inline-img" />

And here is a (correct) proof of Lemma 2, which I stated as correct in the LaTeX document:

<img src="./vibecoding_images/lemma_2_proof.png" class="inline-img" />

This looks very impressive, and in many ways it is. Unfortunately, it can write equally compelling algebraic manipulations to false statements, which you might not notice are erroneous arguments:

<img src="./vibecoding_images/converse_to_lemma_2_fake_proof_1.png" class="inline-img" />

As a mathematician, however, you are trained to notice characteristic ways in which arguments might fail: 


<img src="./vibecoding_images/converse_to_lemma_2_fake_proof_1_response.png" class="inline-img" />

The model seems much better at writing numerical tests than proofs, perhaps because of the preponderance of numerical python code on the internet. I asked the model to write numerical code to test the converse of Lemma 2:

<img src="./vibecoding_images/lemma_2_code_test_initial.png" class="inline-img" />

This produces several pages of code, any error in which would make the test incorrect. Luckily, the model can produce some explanations for its code:

<img src="./vibecoding_images/lemma_2_code_test_explaining.png" class="inline-img" />

These explanations lead, upon careful investigation and some arguing with the model, to bug discovery and subsequent bug-fixes: 

<img src="./vibecoding_images/lemma_2_code_test_bug_1.png" class="inline-img" />

<img src="./vibecoding_images/lemma_2_code_sigma_eval.png" class="inline-img" />

<img src="./vibecoding_images/lemma_2_code_test_bug_2.png" class="inline-img" />

<img src="./vibecoding_images/lemma_2_code_test_bug_2_fix_1.png" class="inline-img" />

<img src="./vibecoding_images/lemma_2_code_test_bug_2_fix_2.png" class="inline-img" />

Finally, the after I feel satisfied with my understanding of the code, the model produces 
some code, which has one more bug upon evaluation which the model can immediately fix from the error message spit out by the python evaluator:

<img src="./vibecoding_images/lemma_2_code_test_final_code.png" class="inline-img" />

*Very impressively*, when this run then ended up producing a candidate counterexample, the model can notice that in fact the counterexample was a false positive, due to insufficiently many samples taken in
the randomized algorithm being used to test for IJK-convexity:

<img src="./vibecoding_images/lemma_2_counterexample.png" class="inline-img" />

<img src="./vibecoding_images/lemma_2_counterexample_2.png" class="inline-img" />


After this, with many samples, no more counterexamples were found! In the second notebook linked above, I worked with the model to upgrade the code to look for higher-dimensional counterexamples and it found none there as well. After this, for the first time, *I changed my 
mind about a mathematical question due to a collaboration with an LLM*! This was a big eureka moment for me.

After this moment, I was positively feeling the AGI. However, all attempts to work with the model to produce a *proof* of the converse of Lemma 2 were absolutely disastrous:

<img src="./vibecoding_images/lemma_2_converse_fake_proofs_begin.png" class="inline-img" />

<img src="./vibecoding_images/lemma_2_converse_fake_proofs_2.png" class="inline-img" />

<img src="./vibecoding_images/lemma_2_converse_fake_proofs_3.png" class="inline-img" />



The model confidently produces large, complex symbol manipulations, which constantly use
unjustified claims, often 'due to symmetry':

<img src="./vibecoding_images/lemma_2_converse_fake_proofs_3.png" class="inline-img" />

<img src="./vibecoding_images/lemma_2_converse_fake_proofs_4.png" class="inline-img" />


The model tries to get into fancy math, by using representation theory:

<img src="./vibecoding_images/lemma_2_converse_fake_proofs_5.png" class="inline-img" />

If you are an interested mathematician, do take a look at the chat log -- it is a fascinating example of the kinds of "characteristic errors" that these models make, and the challenges
that a human has in interpreting what is happening as the model tries to "reason". If you are not a mathematician, it is worth emphasizing that the above screenshots are only very short excerpts of a rather extensive conversation that I had with the model, where the model constantly produced *very impressive-looking mathematical formulae*, almost all of which are difficult to evaluate for correctness, and always ended up *completely falling apart* due to trivial errors in reasoning/symbolic manipulation/whatever it is that the model does. 

In the end, all of the model's attempts to produce a proof *fail completely*, despite an extensive attempt at helping the model
to think more carefully. I tried, in the second notebook, to get the model to write
symbolic algebra code to force it to make correct symbolic manipulations, and this attempt
of mine failed miserably -- it was not able to consistly generate symbolic algebra code
reliably enough for the process to be worth debugging. After some additional extensive efforts,
I decided that the model is good at numerical coding at proving the lemma I was interested in, and gave up. I think that a model that can (through an interactive conversation) prove the 
converse to Lemma 2 (*not that I currently have a proof -- but the numerics are convincing!*)
would make it into my regular mathematical research toolkit, and would significantly change the way that I work day-to-day.


The workflow above immediately suggests some human-interface improvements which would make this kind of AI-assisted research much more effective: 
...
1.	Panes, "please fix the error", and other vibecoding tropes.
	...
	For example, it would be a huge improvement on the workflow described above to
	...
	- Both documents linked above were generated in about 2 hours. They are quite slow to load. *AI assisted mathematics very quickly produces large documents, which need tooling to me made navigable*.
...
<br/>
<details>
<summary> Mathematicians are often concerned with finding and articulating the right concepts, statements, definitions, and beliefs, rather than with the correctness of any particular claim.</summary>
...
<br/>
<details>
<summary> Mathematical values center human understanding, and even human joy, as the purpose of the activity.
</summary>
...
<br/>
<details>
<summary> Human proofs and symbolic calculations are analogous to tests in software engineering. </summary>
...
<summary> Informal, symbolic *techniques* endowed with a rich set of associations and intuitions can be just as important as formal proofs, and indeed can significantly outrank formal proofs in their mathematical importance.</summary>
...
[^ias]: Indeed, at the IAS conference where I tested this workflow, someone thought about doing just that.]]
